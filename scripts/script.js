...
function cPush() {
    ...
    if (cStep < cPushArray.length) {
        ...
        checkLayerArray.length = cStep
    }
    if (cStep > cLimit) {
        cStep = cLimit;
        cPushArray = cPushArray.slice(1);
        checkLayerArray = checkLayerArray.slice(1);
    }
    checkLayerArray.push(layer.id);
...
function cUndo() {
    if (0 < cStep) {
        ...
        canvasPic.onload = function () {
            ...
            if(layer.id == checkLayerArray[cStep]){
                layer_ctx.drawImage(canvasPic, 0, 0);
            }
            layers(checkLayerArray[cStep]);
        }
    }
}
...
function check(e) {
    ...
    switch (code) {
        ...
        case 190: alphaSlider.value = parseFloat(alphaSlider.value) + 0.05;
            console.log(alphaSlider.value);
            brushAlpha = alphaSlider.value; break;
        case 49: layers('layer1'); break;
        case 50: layers('layer2'); break;
        case 51: layers('layer3'); break;
